...
In this post, we'll walk through the full statistical machinery behind an A/B test: modeling the data, estimating the treatment effect, and testing whether the difference is "real," all via simulation.  Along the way we'll encounter the Law of Large Numbers, the bootstrap, the Central Limit Theorem, and the (sometimes misunderstood) t-test.
...
Early fluctuations are large because each additional observation has a strong influence when the sample is still small. As the sample grows, each new observation has less impact, and the estimate stabilizes near the true effect.
...
Because zero would represent no difference between the two CTAs, checking whether the interval crosses zero gives us a quick way to assess whether the observed effect is distinguishable from random noise.
...
- **Null hypothesis** $H_0: \theta = 0$, meaning the two CTAs produce the same sign-up rate.
- **Alternative hypothesis** $H_1: \theta \neq 0$, meaning the CTAs differ.
...
The result is striking.  Instead of the nominal 5%, the false positive rate under peeking is roughly **two to three times higher**: in about 15% or more of experiments where there is *no real effect*, the peeking strategy would lead us to incorrectly declare a winner.
